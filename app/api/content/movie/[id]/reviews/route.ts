...
export async function GET(
  ...
  try {
    ...
    logError(error, 'Get movie reviews', undefined, `/api/content/movie/${params.id}/reviews`)
...
export async function POST(
  ...
  try {
    ...
    // Verify movie exists - but allow reviews for mock data (fallback to localStorage)
    const movie = await prisma.movie.findUnique({
      where: { id: params.id },
    })

    if (!movie) {
      // Movie not in database - this might be mock data
      // Return a more helpful message
      return NextResponse.json(
        { message: 'Movie not found in database. This content may not be available yet.' },
        { status: 404 }
      )
    }
    ...
    if (error.code === 'P2003') {
      return NextResponse.json(
        { message: 'Movie not found in database', code: 'NOT_FOUND' },
        { status: 404 }
      )
    }
    ...
    if (error.code === 'P2002') {
      return NextResponse.json(
        { message: 'You have already reviewed this movie', code: 'DUPLICATE_REVIEW' },
        { status: 409 }
      )
    }
