...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check cache
    const cacheKey = `series:${featured}:${limit}:${offset}`
    ...
    const series = await prisma.series.findMany({
      ...
      console.error('Prisma query error in GET /api/content/series:', prismaError)
    ...
    // Cache for 5 minutes
    await cacheService.set(cacheKey, series, { ttl: 300 })
    ...
    console.error('Error in GET /api/content/series:', error)
    const errorMessage = error instanceof Error ? error.message : 'Unknown error'
    const errorStack = error instanceof Error ? error.stack : undefined
    console.error('Error details:', { errorMessage, errorStack })
    
    logError(error, 'Get series', undefined, '/api/content/series')
    const errorInfo = handleError(error)
    return NextResponse.json(
      { message: errorInfo.message || 'Failed to fetch series', code: errorInfo.code },
